...
# # Compute paths relative to the script's location
...
# # Go up one directory to reach the 'python/' folder
# python_dir = os.path.dirname(current_script_dir)  # .../python
...
a = renderInterface.AnimationHandler("pelican://osg-htc.org/nasa/nsdf/climate1/llc4320/idx/theta/theta_llc4320_x_y_depth.idx", pathType = "visuspy")

# set render info
a.setDataDim(17280, 12960, 90, 10311);
...
z_range = [0, a.z_max]
t_list = np.arange(0, 24*10, 4, dtype=int).tolist()
quality=0
...
# a.generateScript(input_names, kf_interval, dims, meshType, world_bbx_len, cam, tf_range, template="fixedCam", outfile="case1_script");
