...
data = a.readData(t=t_list[0], x_range=x_range, y_range=y_range,z_range=z_range, q=quality, flip_axis=2, transpose=True)
...
meshType = "structured"
bgImg = os.path.join(os.path.dirname(sys.path[0]), 'renderingApps', 'mesh', 'earth_flat_map.png')
...
a.generateScript(input_names, kf_interval, dims, meshType, world_bbx_len, cam, tf_range, template="fixedCam", outfile=outputName, bgImg=bgImg);
...
if (args.save or args.save_only):
    ...
    a.saveRawFilesByVisusRead(t_list=t_list, x_range=x_range, y_range=y_range,z_range=z_range, q=quality, flip_axis=2, transpose=True, output_dir=output_dir)
