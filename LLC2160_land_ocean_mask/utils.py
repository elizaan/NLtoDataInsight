...
def compute_land_ocean_mask3d(
    ...
    # Build optional read kwargs for spatial subset
    read_kwargs = {}
    if x_range is not None:
        read_kwargs['x'] = x_range
    if y_range is not None:
        read_kwargs['y'] = y_range
    if z_range is not None:
        # Do not pass full z_range for surface reads; handled below.
        pass

    if not use_surface_only:
        # Full 3D read (apply spatial subset if provided)
        if z_range is not None:
            read_kwargs['z'] = z_range
        salt3d = db_salt.read(time=time, quality=quality, **read_kwargs)  # (nz, ny, nx)
        mask3d = (salt3d > eps).astype(np.uint8)
        return mask3d

    # Surface-only path: read surface layer at the requested x/y subset
    # If a z_range is provided, broadcast the surface mask to the requested
    # number of depth levels; otherwise broadcast to the full dataset depth.
    surface_z = [0, 1]
    if z_range is not None:
        # request the top-most level of the requested z_range
        surface_z = [z_range[0], z_range[0] + 1]

    salt_surface = db_salt.read(time=time, quality=quality, z=surface_z, **read_kwargs)  # (1, ny, nx) or (k, ny, nx)
    # ensure we have a 2D top surface
    mask2d = (salt_surface[0] > eps).astype(np.uint8)

    # determine number of z levels to broadcast to
    if z_range is not None:
        nz = max(1, int(z_range[1] - z_range[0]))
    else:
        # try to get full depth by reading a small sample of full 3D if needed
        try:
            salt_full = db_salt.read(time=time, quality=quality, x=read_kwargs.get('x'), y=read_kwargs.get('y'))
            nz = salt_full.shape[0]
        except Exception:
            nz = 1
