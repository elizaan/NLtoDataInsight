...
if (typeof window !== 'undefined') window.updateArtifactsPanelLayout = updateArtifactsPanelLayout;
    ...
    async sendMessage() {
        ...
        } else if (this.conversationState === 'awaiting_clarification') {
            // This message is an answer to the clarifying questions; tag it so backend may treat differently if desired
            await this.sendApiMessage(message, 'continue_conversation', {clarify_response: true});
    ...
    handleProgressMessage(msg) {
        ...
        switch (msg.type) {
            case 'intent_parsed':
                ...
                // ONLY show this if NOT awaiting clarification (to avoid duplicates)
            ...
            case 'insight_analysis':
                // Show intermediate analysis from the insight extractor
                try {
            ...
            case 'insight_raw':
                // Raw LLM output - skip showing this to users, it's internal/verbose
                // The parsed structured analysis is shown via 'insight_analysis' above
                break;
            ...
            case 'insight_generated':
                ...
                if (msg.data && msg.data.data_summary) {
                    // Check if data_summary is a non-empty object or a non-empty string
                    const isEmpty = (typeof msg.data.data_summary === 'object' && Object.keys(msg.data.data_summary).length === 0)
                        || (typeof msg.data.data_summary === 'string' && msg.data.data_summary.trim() === '');
            ...
            case 'clarifying_questions':
                // Show clarifying questions generated by the intent parser
                try {
                    const q = msg.data || {};
                    if (Array.isArray(q.clarifying_questions) && q.clarifying_questions.length > 0) {
                        // Add friendly header before questions
                        this.addMessage('🤔 I have a few quick questions to clarify your request:', 'bot');
                        q.clarifying_questions.forEach((qq) => this.addMessage(`• ${qq}`, 'bot'));
                        // Mark conversation state so next user reply is treated as clarification answer
                        this.conversationState = 'awaiting_clarification';
                        // Store the last clarifying payload for reference
                        this.lastClarifying = q;
                        this.updateStatus('Awaiting clarification', 'info');
                    } else {
                        // No explicit questions: show notes if present
                        if (q.notes) this.addMessage(q.notes, 'bot');
                    }
                } catch (e) {
                    console.warn('Failed to render clarifying questions', e);
                }
                break;
        ...
        try {
            ...
            if (data.assistant_messages && Array.isArray(data.assistant_messages)) {
                ...
                data.assistant_messages.forEach((msg, index) => {
                    ...
                    if (msg.type === 'intent_parsing' && msg.data) {
                        // Only show intent if NOT awaiting clarification (to avoid duplicates with clarifying_questions progress msg)
                        if (!msg.data.awaiting_clarification) {
                        ...
                        // DON'T render clarifying questions here - they're already shown via progress message
            ...
            switch (data.type) {
                ...
                case 'particular_response':
                    ...
                    // Only show data_summary if not empty
                    if (data.data_summary) {
                        const isEmpty = (typeof data.data_summary === 'object' && Object.keys(data.data_summary).length === 0)
                            || (typeof data.data_summary === 'string' && data.data_summary.trim() === '');
