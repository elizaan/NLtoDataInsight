...
# Common color scale for May/Nov for fair comparison
vmin_vals = np.nanmin([finite_may.min() if finite_may.size else np.nan,
                       finite_nov.min() if finite_nov.size else np.nan])
vmax_vals = np.nanmax([finite_may.max() if finite_may.size else np.nan,
                       finite_nov.max() if finite_nov.size else np.nan])
if not np.isfinite(vmin_vals) or not np.isfinite(vmax_vals) or vmin_vals == vmax_vals:
    vmin_vals, vmax_vals = -2.0, 35.0

# Symmetric color scale for anomaly
max_abs_anom = np.nanmax(np.abs(finite_anom)) if finite_anom.size else 1.0
...
plt.title(f"Near-surface temperature (z=0)\nIndian Ocean — {date_may} (Q={quality})")
plt.xlabel("Longitude (°E)")
plt.ylabel("Latitude (°N)")
# Add subtle stats as text box
txt = fmt_stats_row("May", stats_may)
plt.gca().text(0.01, 0.02, txt, transform=plt.gca().transAxes,
               fontsize=9, va="bottom", ha="left",
               bbox=dict(facecolor='white', alpha=0.7, edgecolor='none'))
out1 = os.path.join(plots_dir, "plot_1_20251202_190204.png")
...
cb = plt.colorbar(im, pad=0.02)
cb.set_label(f"{variable_name} ({variable_unit})")
plt.title(f"Near-surface temperature (z=0)\nIndian Ocean — {date_nov} (Q={quality})")
plt.xlabel("Longitude (°E)")
plt.ylabel("Latitude (°N)")
...
plt.gca().text(0.01, 0.02, txt, transform=plt.gca().transAxes,
               fontsize=9, va="bottom", ha="left",
               bbox=dict(facecolor='white', alpha=0.7, edgecolor='none'))
out2 = os.path.join(plots_dir, "plot_2_20251202_190204.png")
...
cb = plt.colorbar(im, pad=0.02)
cb.set_label(f"Temperature anomaly (°C)\n(Nov - May)")
plt.title(f"Near-surface temperature anomaly (z=0)\nIndian Ocean — {date_nov} minus {date_may} (Q={quality})")
plt.xlabel("Longitude (°E)")
plt.ylabel("Latitude (°N)")
...
plt.gca().text(0.01, 0.02, txt, transform=plt.gca().transAxes,
               fontsize=9, va="bottom", ha="left",
               bbox=dict(facecolor='white', alpha=0.7, edgecolor='none'))
out3 = os.path.join(plots_dir, "plot_3_20251202_190204.png")
plt.savefig(out3, dpi=150, bbox_inches="tight")
plt.close()

# Plot 4: Histograms with percentile markers
plt.figure(figsize=(10, 6))
...
def add_percentile_lines(values, label_prefix, color):
    ...
    # Annotate median
    plt.text(p50, plt.ylim()[1]*0.95, f"{label_prefix} median={p50:.2f}°C",
...
plt.legend()
out4 = os.path.join(plots_dir, "plot_4_20251202_190204.png")
...
table.scale(1, 1.5)
plt.title("Summary statistics across Indian Ocean domain (ocean-only; z=0; Q={})".format(quality), pad=10)
out5 = os.path.join(plots_dir, "plot_5_20251202_190204.png")
plt.savefig(out5, dpi=150, bbox_inches="tight")
plt.close()

print(json.dumps({
    "status": "success",
    "plots_created": [out1, out2, out3, out4, out5],
    "notes": "Maps use imshow with origin='lower' and extent mapped to lon/lat bounds; surface layer (z=0); consistent color scales for May/Nov and symmetric for anomaly; histograms include percentile markers."
}))